...
module.exports = class Command {
    ...
    validate(message, args = []) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (_cmd.requireRole !== '') {
                if (_cmd.onlyGuildId) {
                    var result = this.checkGuildRole(message, _cmd.onlyGuildId, _cmd.requireRole);
                    ...
                    request.get('user/' + message.author.id,
                        (json) => {
                            var result = this.checkGuildRole(message, json.user.orgId, _cmd.requireRole);
                            if (typeof result === 'string') {
                                return reject(result);
                            }
                            else {
    ...
    checkGuildRole(message, guildId, requireRole) {
        const guild = client.guilds.cache.find(g => g.id === guildId);
        const requiredRole = guild.roles.cache.find(r => r.name === requireRole);
        if (!requiredRole) {
            return 'sorry I can not do that. Your server does not have an admin role!';
        }
        const member = guild.members.cache.find(u => u.id === message.author.id);
        if (member.roles.cache.has(requiredRole.id)) {
            return {
            ...
            return 'sorry, but you are not an admin in your organization!';
        }
    }

    execute(message, args) {
        log('You need to implement what your command "' + this.name + '" needs to do!');
    }
    ...
    handleConversation(message, args, dataMessage) {
        ...
        if (data.values[data.prev] !== undefined) {
            // Validation of answer
            if (Array.isArray(data.validation) && !data.validation.includes(Number(message.content))) {
                log('Answer is not a valid value', message.content, data.validation);
                return this.sendNextQuestion(message,
                    'You did not give a valid answer!',
                    'Please look at the question again and give me a correct answer.', data, 'RED');
            }
    ...
    static async handleReaction(event, reaction, user) {
        ...
        log('handling reaction', event, reaction.emoji.name, user.username);
        // When we receive a reaction we check if the reaction is partial or not
        if (reaction.partial) {
            log('Partial reaction received, fetching it!');
            // If the message this reaction belongs to was removed the fetching might result in an API error, which we need to handle
            try {
                await reaction.fetch();
            } catch (error) {
                log('Something went wrong when fetching the message: ', error);
                // Return as `reaction.message.author` may be undefined/null
                return;
            }
        }
        if (reaction.message.partial) {
            log('Partial message received, fetching it!');
            // If the message this reaction belongs to was removed the fetching might result in an API error, which we need to handle
            try {
        ...
        if (reaction.message.author.id !== client.user.id || user.id === client.user.id) {
            return; // My own, so do not care
        }
        await reaction.message.suppressEmbeds(false);
        var data = Command.decodeFooter(reaction.message);
        if (data === false) {
            log('No data found for reaction, trying to fetch it...', reaction.message.id);
            console.log(await reaction.message.channel.messages.fetch(reaction.message.id, false));
        }
        var data = Command.decodeFooter(reaction.message);
        if (data === false) {
            log('No data found for reaction so removing it', reaction);
            reaction.users.remove(user);
        }
        else if (data && data.command !== undefined) {
            var command = client.commands.get(data.command);
            if (command && command.executeReaction) {
                command.setNameUsed(data.command);
                command.executeReaction(event, reaction, user, data);
                ...
                log('Command does not have an executeReaction method', data.command);
            }
        }
        else {
            log('Data does contain a command name', data);
        }
    }
}
>>>>>>> upstream/master
